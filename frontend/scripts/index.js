const url = "http://localhost:2400" ||"https://map-twit-be.herokuapp.com"
let userInfo;
let data;
let mainDiv;
let imgSource;
...
document.getElementById('submitbtn').addEventListener('click', (e) => {
	...
	fetch(`${url}/userInfo/${screennameText.value}`)
	    .then((res) => res.json())
	    .then((res) => {
			userInfo = res;
...
function getInitialData(screenName) {
	fetch(`${url}/followers/${screenName}`)
	    .then((res) => res.json())
	    .then((res) => {
	      data = res;
	    })
...
function getFollowing() {
	if (!fetching) {
		...
		fetch(`${url}/following/${userInfo["screenName"]}`)
		  .then((res) => res.json())
		  .then((res) => {
		    data = res;
		  })
...
function getFollowers() {
	if (!fetching) {	
		...
		fetch(`${url}/followers/${userInfo["screenName"]}`)
		  .then((res) => res.json())
		  .then((res) => {
		    data = res;
		  })
